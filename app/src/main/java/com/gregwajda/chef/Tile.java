...
import com.gregwajda.chef.GameScreen;
...
public class Tile {
    ...
    public void checkSideCollision(Rect rleft, Rect rright, Rect leftfoot, Rect rightfoot) {
        if (type != 5 && type != 2 && type != 0 && type != 7){
            if (Rect.intersects(rleft, r)) {
                robot.setCenterX(tileX + 127);
    
                robot.setSpeedX(0);
    
            }else if (Rect.intersects(leftfoot, r)) {
                
                robot.setCenterX(tileX + 95);
                robot.setSpeedX(0);
            }
            
            if (Rect.intersects(rright, r)) {
                robot.setCenterX(tileX - 37);
    
                robot.setSpeedX(0);
            }
            
            else if (Rect.intersects(rightfoot, r)) {
                robot.setCenterX(tileX - 35);
                robot.setSpeedX(0);
            }
        }
    }
